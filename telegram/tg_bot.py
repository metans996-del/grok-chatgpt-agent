...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "approve":
        await query.edit_message_text(text=f"PR approved by {query.from_user.full_name}\n{pr}")
    else:
        await query.edit_message_text(text=f"PR rejected by {query.from_user.full_name}\n{pr}")
